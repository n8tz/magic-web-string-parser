		...
		selected.forEach(
			node => {
				...
				if ( node?.key ) {
					oKey = node.key;
					try {
						node.key = is.function(value) ? value(node.key, node.__path) : value;
					} catch ( e ) {
					...
					try {
						if ( raw )
							node.newRawValue = is.function(value) ? value(node.data, node.__path) : value;
						else
							node.newValue = is.function(value) ? value(node.data, node.__path) : value;
					} catch ( e ) {
...
export function parse( data, options, context ) {
	...
	do {
		...
		if ( isNative )
			output.totalNativeTypeNodes++;
		else if ( codecs[current.type] )
			...
			output.totalNodesWeight += current.weight;
