	...
	printVars( usePath ) {
		...
		while ( stack.length ) {
			...
			if ( cNode.isPropValueOf ) {
				output.push((usePath
				             ? cNode.__humanPath
				             : cNode.isPropValueOf) + Array(Math.max(20 - cNode.isPropValueOf.length, 2)).join(" ") + "= " +
	...
	do {
		...
		if ( current.isPropValueOf ) {
			...
			if ( isNative )
				...
				currentJsonPathObj.value                                 = [
					{
						...
						[current.type]: [nextJsonPathObj = {
							__path     : currentJsonPathObj.__path + ".value" + "." + current.type + "[0]",
							__humanPath: (currentJsonPathObj.__humanPath) + "/" + current.isPropValueOf + (codecs[current.type]
							                                                                               ? "/" + current.type
				...
				output.nodesByPath[currentJsonPathObj.__path + ".value"] = current;
			}
		}
		if ( !current.isPropValueOf ) {
			...
			currentJsonPathObj[current.type].push(
				nextJsonPathObj = {
					...
					__humanPath: (currentJsonPathObj.__humanPath) + (codecs[current.type]
					                                                 ? "/" + current.type
					                                                 : current.type.endsWith("Item")
